...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn_out = self.c_attn(x) + self.c_attn_lora(x)
        ...
        # output projection
        proj_out = self.c_proj(y) + self.c_proj_lora(y)
...
class LoRALinear(nn.Module):
    def __init__(self, input_dim, output_dim, lora_rank, alpha=0.5):
        super(LoRALinear, self).__init__()
        self.lora_rank = lora_rank
        n = torch.distributions.Normal(torch.tensor([0.0]), torch.tensor([1/torch.sqrt(torch.tensor(lora_rank)).float()]))
        self.a = nn.Parameter(n.sample((lora_rank, output_dim)).squeeze(2))
        self.b = nn.Parameter(torch.zeros(input_dim, lora_rank))
        self.alpha = alpha
    ...
    def forward(self, x):
        ...
        return x @ delta_w.to(device) * self.alpha
...
class DistilRNN(nn.Module):
    def __init__(self, hidden_size=128, embedding_length=512):
        ...
        self.word_embeddings = nn.Embedding(GPTConfig.vocab_size, embedding_length)
    ...
    def forward(self, input_sentence):
        ...
        h_0 = torch.autograd.Variable(torch.zeros(1, input_sentence.shape[0], self.hidden_size).cuda()) # Initial hidden state of the LSTM
        c_0 = torch.autograd.Variable(torch.zeros(1, input_sentence.shape[0], self.hidden_size).cuda())
